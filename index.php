...
<head>
	...
	<!-- Knockout -->
	<script src="<?= $rel3rdPath ?>js/knockout.js"></script>
	<!-- 3rd -->
...
<body>
	...
	<nav class="navbar navbar-inverse navbar-fixed-top">
		<div class="container">
			...
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="javascript:window.location=scriptLocation();"><?= $title ?> URL</a>
			</div>
	...
	<div class="container" role="main">
		...
		<div data-bind="visible: showNewUrl" class="alert alert-success" role="alert">
			<!-- <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> -->
			<h4>Encrypted:</h4>
			<a data-bind="attr: {href: urlString}" target="_blank"> <span data-bind="text: urlString"
					style="font-weight: bold;"></span>
			</a> <br />
			<br />

			<button data-bind="visible: shortUrlButton, click: getShortUrl" type="button"
			...
			<span data-bind="visible: shortUrlSpan">
				<h4>Shorturl:</h4> <a data-bind="attr: {href: shortUrlString}" target="_blank"> <span
						data-bind="text: shortUrlString" style="font-weight: bold;"></span>
				</a>
			</span>

			<div align="right">
				<a data-bind="attr: {href: deleteUrl}" target="_self"> <span class="glyphicon glyphicon-trash"></span>
				</a>
			</div>
		</div>

		<div data-bind="visible: errorBox" class="alert alert-danger" role="alert">
			<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
			<span class="sr-only">Error:</span> <span data-bind="text: errorText"></span>
		</div>

		<div data-bind="visible: statusBox" class="alert alert-info" role="alert">
			<span data-bind="visible: infoTextSpin" class="glyphicon glyphicon-refresh glyphicon-spin"
		...
		<form class="form" data-bind="visible: showForm">
			...
			<label for="inputUrl" class="sr-only">http://</label> <input data-bind="textInput: rawUrlString, value: enterText, valueUpdate: 'afterkeydown', 
	event: { keypress: enterKeyboardCmd}" type="url" id="inputUrl" class="form-control" placeholder="http://" required
				autofocus>
			<button data-bind="click: generateUrl" class="btn btn-lg btn-primary btn-block" type="submit">GET</button>
		</form>
		<p>&nbsp;</p>

		<footer>
			<cite>Encrypted urls for one day!</cite> | <a href="https://github.com/dapkdapk/p1n"
	...
	<div id="cipherdata" style="display: none;">
		<?= $CIPHERDATA ?>
	</div>

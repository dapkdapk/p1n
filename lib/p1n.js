...
var clickGenerateUrl = function() {
	if (v.rawUrlString() == undefined || v.rawUrlString() == ""
			|| !ko.utils.stringStartsWith(v.rawUrlString(), "http")) {
...
var sendData = function(rawUrlString) {
	if (!sjcl.random.isReady()) {
		showBox('Sending url (Please move your mouse for more entropy)...',
				true);
	...
	var randomkey = sjcl.codec.base64
			.fromBits(sjcl.random.randomWords(8, 0), 0);
	...
	$.post(scriptLocation(), data_to_send, 'json').error(function() {
		showError('Url could not be sent (server error or not responding).');
			...
			function(data) {
				if (data.status == 0) {
					...
					showError('Could not create short url: ' + data.message);
				} else {
...
$(function() {
	...
	if ($('div#cipherdata').text().length > 1) {
		...
		if (window.location.hash.length == 0) {
			showError('Cannot decrypt short url: Decryption key missing in URL (Did you use a redirector or an URL shortener which strips part of the URL ?)');
			return;
		}
		var messages = jQuery.parseJSON($('div#cipherdata').text());
		showStatus("One moment please.. try to open url.");
